...
import { changeOwner, deleteFile, GetFiles, getSignedUrl, makePublic, saveMetaDesc } from '../service/upload';
...
const ViewSessionsScreen = (props) => {
  ...
  const ListFiles = async () => {
    GetFiles(props.token).then(data => {
                ...
                <View>
                  <View style={GlobalStyles.View_Flex}>
                    ...
                    <Text style={[GlobalStyles.color, GlobalStyles.Font,{flex: 1, textAlign: 'right'}]}>
                      {item.user_created == props.userId.toString() ? `${item.user_sent_name ? 'session with ' + item.user_sent_name : 'solo session'}` : 'session with ' + item.user_created_name}
                    </Text>
                  </View>
                  <View>
                    <View style={GlobalStyles.View_Flex}>
                      ...
                      <TouchableOpacity disabled={(item.public == true || item.owner_id !== props.userId) ? true : false} style={item.public || item.owner_id !== props.userId ? GlobalStyles.InputDisabled : GlobalStyles.Transparent_Input_View}
                        onPress={() => { makePublicFile(item.file_path) }}>
                        <Text style={[(item.public || item.owner_id !== props.userId) ? GlobalStyles.InputTextDisabled : GlobalStyles.InputText, GlobalStyles.Font]}>
                          make it public
                        </Text>
                      </TouchableOpacity>
                    </View>
                    <View style={GlobalStyles.View_Flex}>
                      <TouchableOpacity disabled={item.owner_id !== props.userId ? true : false} style={item.owner_id !== props.userId ? GlobalStyles.InputDisabled : GlobalStyles.Transparent_Input_View} onPress={() => { setSelRecord(item.recording_id); setEditMeta(true) }}>
                        <Text style={[item.owner_id !== props.userId ? GlobalStyles.InputTextDisabled : GlobalStyles.InputText, GlobalStyles.Font, styles.padding, styles.padding]}>edit</Text>
                      </TouchableOpacity>
                      {item.user_sent_name && <TouchableOpacity disabled={(item.owner_id !== props.userId) ? true : false} style={item.owner_id !== props.userId ? GlobalStyles.InputDisabled : GlobalStyles.Transparent_Input_View}
                        onPress={() => { changeRecordingOwner(item.recording_id, item.owner_id == item.user_created ? item.user_sent : item.user_created) }}>
                        <Text style={[item.owner_id !== props.userId ? GlobalStyles.InputTextDisabled : GlobalStyles.InputText, GlobalStyles.Font]}>
                          transfer rights
                        </Text>
                      </TouchableOpacity>}
                      <TouchableOpacity disabled={item.owner_id !== props.userId ? true : false} style={item.owner_id !== props.userId ? GlobalStyles.InputDisabled : GlobalStyles.Transparent_Input_View} onPress={() => { setDeleteConfirm(true), setDeleteRecordId(item.recording_id) }}>
                        <Text style={[item.owner_id !== props.userId ? GlobalStyles.InputTextDisabled : GlobalStyles.InputText, GlobalStyles.Font, styles.padding, styles.padding]}>
                          delete
                        </Text>
                      </TouchableOpacity>
                    </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(ViewSessionsScreen);
