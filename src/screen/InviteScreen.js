...
import { sendFriendRequest } from '../service/upload';
...
const InviteScreen = (props) => {
  ...
  const sendRequest = () => {
    console.log(email);
    sendFriendRequest(props.token, email).then(res => {
      setMessage(res.msg ? res.msg : 'friend request sent');
      setShowPopup(true);
    }).catch(err => {
      console.log(err);
      setMessage(err.msg ? err.msg : 'something went wrong. please retry');
      setShowPopup(true);
    });
  ...
  return (
    <ScrollView>
      ...
      <View style={GlobalStyles.Invite}>
        <Text style={GlobalStyles.Invite_Text}>
          enter the email
        </Text>
        <TextInput
      ...
      <TouchableHighlight
        style={GlobalStyles.ButtonEnter}
        onPress={() => sendRequest()}
